...
type DraftBatch = {
  ...
  loose_strips: string; loose_pieces: string;
...
const emptyBatch = (): DraftBatch => ({
  ...
  boxes: "", strips_per_box: "", pieces_per_strip: "", loose_strips: "0", loose_pieces: "0",
...
export default function AddMedicinePage({ onToast, onClose, onSaved, defaultTab }: Props) {
  ...
  const piecesPerStripNum = Math.max(1, parseInt(batch.pieces_per_strip, 10) || 1);
  const totalPieces = isBoxUnit
    ? (parseInt(batch.boxes || "0", 10) * piecesPerBox)
      + (parseInt(batch.loose_strips || "0", 10) * piecesPerStripNum)
      + parseInt(batch.loose_pieces || "0", 10)
      ...
      for (const item of queue) {
        await createMedicineWithBatch({
          ...
          batch: {
            ...
            strips_per_box:   item.isBoxUnit ? Math.max(1, parseInt(item.batch.strips_per_box || "1", 10)) : 1,
            pieces_per_strip: item.isBoxUnit ? Math.max(1, parseInt(item.batch.pieces_per_strip || "1", 10)) : 1,
            loose_strips:     item.isBoxUnit ? parseInt(item.batch.loose_strips || "0", 10) : 0,
