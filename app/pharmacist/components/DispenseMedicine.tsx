...
/** Given a total piece count and the batch's packaging info (strips per
 *  box, pieces per strip — both optional/nullable, matching Warehouse's
 *  model), work out how many complete boxes and how many loose pieces that
 *  total represents. If the box size isn't known (either factor is
 *  null/zero), everything is just loose pieces — there's no way to know
 *  what "one box" means for this batch. */
function splitIntoBoxPiece(total: number, stripsPerBox: number | null, piecesPerStrip: number | null) {
  const piecesPerBox = Math.max(0, (stripsPerBox ?? 0) * (piecesPerStrip ?? 0));
  if (piecesPerBox <= 0) return { boxes: 0, loosePieces: total };
  const boxes = Math.floor(total / piecesPerBox);
  const loosePieces = total % piecesPerBox;
...
export async function dispenseFEFO(
  ...
  for (const batch of (batches ?? []) as MedicineBatch[]) {
    ...
    const { boxes: newBoxes, loosePieces: newLoose } =
      splitIntoBoxPiece(newTotal, batch.strips_per_box, batch.pieces_per_strip);
    ...
    const { error: updErr } = await supabase.from("pharma_medicine_batches")
      .update({ boxes: newBoxes, loose_pieces: newLoose, status: newStatus })
...
export default function DispenseMedicineModal({ medicine, onClose, onSaved, onToast }: Props) {
  ...
  const piecesPerBox = selectedBatch ? Math.max(1, (selectedBatch.strips_per_box ?? 0) * (selectedBatch.pieces_per_strip ?? 0)) : 1;
  ...
  const { boxes: newBoxes, loosePieces: newLoose } = selectedBatch
          ...
          {loadingBatches ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {batches.map(b => {
                ...
                const soon = days <= 30;
                const bPiecesPerBox = Math.max(1, (b.strips_per_box ?? 0) * (b.pieces_per_strip ?? 0));
                return (
                  <button key={b.batch_id} onClick={() => setSelectedBatchId(b.batch_id)} style={{
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 11, color: t.text3, marginTop: 1 }}>
                        Exp {b.expiration_date || "—"} · {b.boxes} box{b.boxes !== 1 ? "es" : ""} × {bPiecesPerBox} + {b.loose_pieces} loose
                      </div>
                    </div>
