...
/* ══════════════════════════════════════════════════════════════════════════
   ...
   THIS PASS: TRUE 3-level packaging (Box -> Strip -> Piece). A batch now
           tracks strips_per_box / pieces_per_strip as FIXED packaging
           factors, and boxes / loose_strips / loose_pieces as real,
           independently depletable stock counts — all three shrink
           correctly as a batch gets dispensed (see DispenseMedicine.tsx's
           splitIntoBoxStripPiece()). pieces_per_box is kept as a
           DB-trigger-maintained mirror of strips_per_box * pieces_per_strip
           for backward compatibility with any older code path that still
           reads it directly.
   ══════════════════════════════════════════════════════════════════════ */
...
/** One received batch of stock — pharma_medicine_batches. */
export type MedicineBatch = {
  ...
  boxes:              number;
  /** FIXED packaging factor — how many strips make up one full box. */
  strips_per_box:     number;
  /** FIXED packaging factor — how many pieces make up one full strip. */
  pieces_per_strip:   number;
  /** Trigger-maintained mirror of strips_per_box * pieces_per_strip, kept
   *  for backward compatibility with older code that reads this directly. */
  pieces_per_box:     number;
  /** Complete, unopened strips sitting outside a full box. Depletes on dispense. */
  loose_strips:       number;
  /** Individual pieces sitting outside a full strip. Depletes on dispense. */
  loose_pieces:       number;
  /** DB-maintained (trigger): boxes*strips_per_box*pieces_per_strip +
   *  loose_strips*pieces_per_strip + loose_pieces. Read-only. */
